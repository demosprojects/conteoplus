...
export function onAuthChange(callback) {
    // A diferencia de onAuthStateChanged de Firebase, onAuthStateChange de
    // Supabase YA dispara solo, con un evento "INITIAL_SESSION", apenas te
    // suscribís — no hace falta (y de hecho rompe) pedir getSession() aparte
    // acá arriba. Si se llaman las dos, app.js recibe el callback dos veces
    // con dos objetos "user" distintos para la misma sesión, y eso rompe la
    // comparación por referencia que hace app.js (currentUser !== user) para
    // detectar que se deslogueó mientras esperaba una respuesta async.
...
export async function obtenerCatalogo(uid) {
    ...
    return data.map(mapProductoDesdeDB);
}
...
export async function importarCatalogo(uid, productos) {
    ...
    for (let i = 0; i < productos.length; i += CHUNK) {
        const lote = productos.slice(i, i + CHUNK).map(p => mapProductoParaDB(uid, p));
